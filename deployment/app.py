...
if selected == "Home":
    def main(): 
        ...
        with col2:
            ...
            st.write("")
            st.write("""
            StockMorrow is a website that predict market movement by analyzing daily headline news all over the world. 
            We’re dedicated to create not only the best but the easiest platform for stock traders  to use. 
            We in StockMorrow want to provide additional insight to help u decide on which stock u want to invest. 
...
if selected == "Projects":
    ...
    if st.button("Analysis"):
        ...
        if stock_prediction(txt):
            st.error('Prediction: The Indeks S&P 500 will remain the same or will go down.')
        else:
            st.success('Prediction: The Indeks S&P 500 will go up!')

        html_temp = """
        <div style="background-color: #46D9B8  ;padding:6px; border: 1px solid; border-radius: 9px; margin-bottom:8px;">
        <h4 style="color:black;text-align:center;">recommendation stocks!!</h4>
        </div>
        """
